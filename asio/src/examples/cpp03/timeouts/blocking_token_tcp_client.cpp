...
struct close_after
{
  ...
  // The socket to be cancelled if the operation does not complete in time.
  tcp::socket& socket_;
};
...
public:
  ...
  // The return_type typedef determines the result type of the asynchronous operation's
  // initiating function.
  ...
  return_type get()
  {
    asio::io_context& io_context = token_.socket_.get_executor().context();
    ...
    // the socket.
    io_context.run_for(token_.timeout_);
